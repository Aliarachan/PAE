/*
 * init.c
 * Fitxer amb les funcions d'inicialitzacio dels moduls de comunicacio i timers.
...
void init_UCS(void) {
	...
	UCSCTL0 = 0x00; // Posa DCOx, MODx al minim possible
	...
	/* Seleccio de la font de rellotge: ACLK el Clock extern de 215, SMCLK i MCLK el DCO intern de 16MHz */
...
/**
* Funcio de configuracio de l'USCI en mode UART.
**/
void init_UART(void) {

	UCA0CTL1 |= UCSWRST; //Fem un reset de la USCI i es desactiva
	UCA0CTL0 = 0x00; //UCSYNC=0 mode asincron
	...
	UCA0BR1 = 0; //... HI byte = 0).
	UCA0MCTL = UCOS16; // No necessitem modulacio (divisio entera), pero
// si oversampling => bit 0 = UCOS16 = 1.
			// El Baud rate es BRCLK/16=1MBps
	P3SEL |= 0x30; //I/O funcion: P3.4 = UART0TX transmitir, P3.5 = UART0RX recibir
	P3REN |= 0x20; //amb resistencia activada de pull-up lentrada P3.5
	P3OUT |= 0x20; //
	...
	UCA0CTL1 &= ~UCSWRST; //Reactivem la linia de comunicacions serie
	UCA0IE |= UCRXIE; //Aixo s'ha d'activar nomes quan fem la recepcio

}

/**
 * Funcio d'inicialitzacio del timer A1 per tal de que provoqui una interrupcio cada 10 microsegons
**/
...
/**
 * Funcio d'inicialitzacio del timer B0 per tal de que provoqui una interrupcio cada 100 microsegons
**/
